...
// WhatsApp alerts disabled — blocked on getting Meta message templates approved.
// import { sendOrderWhatsAppAlert, sendOrderStatusWhatsAppAlert } from '@/lib/notifications/whatsapp';
...
// WhatsApp status alert disabled — blocked on getting Meta message templates approved.
// export async function sendOrderStatusAlertAction(orderId, customerName, status) {
//   const result = await sendOrderStatusWhatsAppAlert({ orderId, customerName, status });
//   if (!result.success) {
//     return { error: result.error || 'Could not send WhatsApp notification.' };
//   }
//   return { success: true };
// }
