...
import { addProduct, updateProduct, toggleArchive, deleteProduct, updateProductSequence } from '../queries/products';
...
export async function revalidateProductsAction() {
  ...
  try {
    ...
    const response = await fetch(`${url}/api/revalidate?tag=products&secret=${key}`, {
      method: 'GET',
      cache: 'no-store'
    });
