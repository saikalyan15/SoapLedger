 ...
 *
 * The storefront caches this product list for 24 hours (see
 * healing-soil/src/lib/products.ts). That long TTL exists because every cache
 * miss wakes our Neon compute for a full 5 billed minutes, and because the
 * storefront pages are statically rendered and inherit that TTL. On-demand
 * purging is therefore the primary way the storefront stays fresh — not the TTL.
 *
...
async function pushProductsRevalidation() {
  ...
  try {
    const response = await fetch(`${url}/api/revalidate`, {
      ...
      cache: 'no-store',
      // Bound the wait. Without this a slow or hanging storefront would block
      // the product save behind it for as long as the platform allows.
...
// Mirrors the textarea maxLength values in ProductView.jsx — enforced again
// here since a form's client-side maxLength doesn't stop a direct POST.
// Both fields print in full on the mini sticker (38.1x14mm), so an
// unbounded value would shrink the on-sticker font past legibility.
const MAX_INGREDIENTS_LENGTH = 200;
const MAX_MINI_LABEL_DESCRIPTION_LENGTH = 44;
...
export async function createProductAction(formData) {
  ...
  const data = {
    ...
    ingredients: (formData.get('ingredients') || '').slice(0, MAX_INGREDIENTS_LENGTH),
    slug: slugInput || buildSlug(name),
    short_description: formData.get('short_description') || '',
    mini_label_description: (formData.get('mini_label_description') || '').slice(0, MAX_MINI_LABEL_DESCRIPTION_LENGTH),
    image_url: formData.get('image_url') || '/products/coming-soon.webp',
