...
export async function getThisMonthSnapshot() {
  const [thisMonth, lastMonth, thisExpenses, lastExpenses] = await Promise.all([
    sql`
      SELECT
        COALESCE(SUM(o.order_value), 0) as revenue,
        COUNT(DISTINCT o.id) as orders,
        COALESCE(SUM(CASE WHEN p.base_type = 'Travel' THEN oi.quantity / 5.0 ELSE oi.quantity END), 0) as soaps
      FROM orders o
  ...
  return {
    this_month: {
      revenue:  parseFloat(thisMonth[0].revenue),
      orders:   parseInt(thisMonth[0].orders),
      soaps:    Math.round(parseFloat(thisMonth[0].soaps)),
      expenses: parseFloat(thisExpenses[0].expenses),
    },
    last_month: {
      revenue:  parseFloat(lastMonth[0].revenue),
      orders:   parseInt(lastMonth[0].orders),
      soaps:    Math.round(parseFloat(lastMonth[0].soaps)),
...
export async function getActionableOrders() {
  const result = await sql`
    ...
    FROM orders
    WHERE status IN ('Awaiting Payment', 'Payment Confirmed', 'In Manufacturing', 'Ready to Dispatch', 'Dispatched')
    GROUP BY status
    ORDER BY
      CASE status
        WHEN 'Ready to Dispatch'  THEN 1
        WHEN 'In Manufacturing'   THEN 2
        WHEN 'Payment Confirmed'  THEN 3
  ...
  const result = await sql`
    WITH last_orders AS (
      SELECT DISTINCT ON (o.customer_id)
        ...
        COUNT(o.id) OVER (PARTITION BY o.customer_id) AS order_count
      FROM orders o
    ...
    avg_gap AS (
      SELECT COALESCE(AVG(diff) FILTER (WHERE diff BETWEEN 7 AND 180), 45) AS avg_days
      FROM (
        SELECT
          customer_id,
          order_date - LAG(order_date) OVER (PARTITION BY customer_id ORDER BY order_date) AS diff
    ...
    SELECT
      lo.id, lo.name, lo.phone,
      lo.last_order,
      lo.order_count::integer,
      (CURRENT_DATE - lo.last_order)::integer               AS days_since,
      (ag.avg_days)::integer                                AS avg_reorder_days,
      LEAST(ROUND((CURRENT_DATE - lo.last_order) / ag.avg_days * 100), 200)::integer AS overdue_pct
    FROM last_orders lo
    CROSS JOIN avg_gap ag
    WHERE lo.order_count >= 2
      AND (CURRENT_DATE - lo.last_order) >= ag.avg_days * 0.75
    ORDER BY days_since DESC
    LIMIT 8
  `;
  return result.map(r => ({ ...r, days_since: parseInt(r.days_since), order_count: parseInt(r.order_count), avg_reorder_days: parseInt(r.avg_reorder_days), overdue_pct: parseInt(r.overdue_pct) }));
}
