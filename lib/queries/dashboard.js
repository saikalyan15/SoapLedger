...
function getDateFilter(from, to, column = 'order_date') {
  ...
  if (from && to) return sql`AND ${sql(column)} BETWEEN ${from}::date AND ${to}::date`;
  if (from) return sql`AND ${sql(column)} >= ${from}::date`;
  if (to) return sql`AND ${sql(column)} <= ${to}::date`;
  return sql``;
}

export async function getRevenueKPIs({ from, to }) {
  const dateFilter = getDateFilter(from, to);
  const expenseDateFilter = getDateFilter(from, to, 'expense_date');
  const orderItemDateFilter = getDateFilter(from, to, 'o.order_date');
  ...
  const costStats = await sql`
    WITH period_expenses AS (
      ...
      WHERE ec.type = 'recurring' ${expenseDateFilter}
    ),
    period_sales AS (
      SELECT COALESCE(SUM(oi.quantity), 0) as soaps_sold
      FROM order_items oi
      JOIN orders o ON o.id = oi.order_id
      WHERE o.status IN ('Delivered', 'Dispatched') ${orderItemDateFilter}
    )
    SELECT 
