...
export async function getRevenueKPIs({ from, to }) {
  const stats = await sql`
    WITH period_orders AS (
      SELECT id, status, order_value, order_date
      FROM orders
      WHERE 
    ...
    SELECT 
      ...
      COALESCE(SUM(order_value) FILTER (WHERE status NOT IN ('Order Placed', 'Awaiting Payment', 'Cancelled') AND EXTRACT(YEAR FROM order_date) = EXTRACT(YEAR FROM CURRENT_DATE)), 0) as total_revenue_this_year,
  ...
  const costStats = await sql`
    SELECT
      ...
      COALESCE(SUM(e.amount) FILTER (WHERE EXTRACT(YEAR FROM e.expense_date) = EXTRACT(YEAR FROM CURRENT_DATE)), 0) as total_this_year
    FROM expenses e
  ...
  const totalRevenueThisYear = parseFloat(s.total_revenue_this_year || 0);
  const totalExpenses = parseFloat(c.total_all || 0);
  const totalExpensesThisYear = parseFloat(c.total_this_year || 0);

  return {
    total_revenue: totalRevenue,
    total_revenue_this_year: totalRevenueThisYear,
    total_revenue_before_this_year: totalRevenue - totalRevenueThisYear,
    ...
    total_expenses_this_year: totalExpensesThisYear,
    total_expenses_before_this_year: totalExpenses - totalExpensesThisYear,
  };
}
