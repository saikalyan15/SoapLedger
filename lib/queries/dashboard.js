...
// Blended unit economics use the owner's explicit category-level
// `include_in_cost_price` choices. A 90-day window smooths lumpy ingredient and
// packaging purchases while staying responsive to recent supplier prices.
export async function getUnitEconomics() {
  const [row] = await sql`
    WITH included_costs AS (
      SELECT COALESCE(SUM(e.amount), 0) AS included_spend
      FROM expenses e
      JOIN expense_categories ec ON ec.id = e.category_id
      WHERE ec.include_in_cost_price = TRUE
    ...
    SELECT
      ...
      CASE WHEN soaps > 0 THEN (revenue - included_spend) / soaps ELSE NULL END AS unit_contribution
    FROM included_costs, recognised_revenue, equivalent_units
  `;
  ...
  const unitContribution = row?.unit_contribution == null ? null : parseFloat(row.unit_contribution);
  ...
  return {
    ...
    contribution_margin_pct: avgSellingPrice > 0 && unitContribution != null
      ? (unitContribution / avgSellingPrice) * 100
      : null,
