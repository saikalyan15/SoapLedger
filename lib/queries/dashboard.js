...
export async function getRevenueKPIs({ from, to }) {
  const stats = await sql`
    ...
    SELECT 
      COALESCE(SUM(order_value) FILTER (WHERE status = ANY(${PAID_STATUSES})), 0) as total_revenue,
      ...
      COALESCE((SELECT SUM(quantity) FROM period_items WHERE status = ANY(${PAID_STATUSES})), 0) as total_soaps_sold,
...
export async function getBreakEvenProjection() {
  ...
  if (actualData.length === 0) return { data: [], currentRunRate: 0, gapToClose: 0 };

  // 1. Current run rate = last complete month's revenue
  const lastCompleteMonth = actualData[actualData.length - 1];
  const currentRunRate = parseFloat(lastCompleteMonth.revenue);
  const lastCompleteMonthName = lastCompleteMonth.month.split(' ')[0];

  // 2. Recurring cost run rate = average of last 3 complete months
  const completeRecurring = actualData.slice(-3);
  ...
  // We include ALL history (including current month if any) for the gap
  ...
  // Generate chart data (keeping lines as they are visually, but using flat projection)
  const data = [];
  let cumRevenue = 0;
  let cumExpenses = 0;
  ...
  let projectionDate = new Date(lastCompleteMonth.month_date);
