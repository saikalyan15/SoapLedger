...
// The current schema records ingredient purchases as expenses, but it does not
// link recipe quantities, batch yield, or labour to products. This query makes
// that limitation explicit so the UI never presents cash movement as product
// profit or produces a false break-even date.
export async function getDashboardDataQuality() {
  const [row] = await sql`
    SELECT
      COUNT(*) FILTER (
        WHERE status NOT IN ('Order Placed', 'Awaiting Payment', 'Cancelled')
      )::integer AS recognised_orders,
      COUNT(*) FILTER (
        WHERE status NOT IN ('Order Placed', 'Awaiting Payment', 'Cancelled')
          AND COALESCE(material_cost, 0) > 0
      )::integer AS orders_with_material_cost,
      COALESCE(SUM(material_cost) FILTER (
        WHERE status NOT IN ('Order Placed', 'Awaiting Payment', 'Cancelled')
      ), 0) AS recorded_material_cost
    FROM orders
  `;

  const recognisedOrders = parseInt(row?.recognised_orders || 0);
  const ordersWithMaterialCost = parseInt(row?.orders_with_material_cost || 0);

  return {
    recognised_orders: recognisedOrders,
    orders_with_material_cost: ordersWithMaterialCost,
    material_cost_coverage_pct: recognisedOrders > 0
      ? (ordersWithMaterialCost / recognisedOrders) * 100
      : 0,
    recorded_material_cost: parseFloat(row?.recorded_material_cost || 0),
    cash_view_ready: true,
    profit_view_ready: false,
    break_even_ready: false,
  };
}
