...
import { assertRazorpayCanAdvance } from './payment-guard';
...
export async function updateOrderStatus(id, status) {
  const [order] = await sql`
    SELECT payment_provider, payment_status FROM orders WHERE id = ${id}
  `;
  if (!order) throw new Error('Order not found');
  assertRazorpayCanAdvance(order, status);
...
export async function updateShipmentStatus(id, status) {
  ...
  const [shipment] = await sql`
    SELECT s.order_id, o.payment_provider, o.payment_status
    FROM shipments s
  ...
  assertRazorpayCanAdvance(shipment, status);
