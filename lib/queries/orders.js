...
export async function getAllOrders() {
  return await sql`SELECT * FROM order_summary ORDER BY order_date DESC, created_at DESC`;
}
...
export async function createOrder(customerData, orderData, items) {
  ...
  const [order] = await sql`
    INSERT INTO orders (
      customer_id, order_date, dispatched_at, delivered_at, order_value, shipping_charge, 
      packaging_cost, material_cost, status, notes
    )
    VALUES (
      ${customer.id}, ${orderData.order_date}, ${orderData.dispatched_at}, ${orderData.delivered_at}, ${orderData.order_value}, 
...
export async function updateOrder(id, orderData, items) {
  ...
  await sql`
    UPDATE orders SET
      ...
      dispatched_at = ${orderData.dispatched_at},
...
export async function updateOrderStatus(id, status) {
  ...
  if (status === 'Dispatched') {
    if (!dispatched_at) dispatched_at = today;
  } else if (status === 'Delivered') {
    delivered_at = today;
    if (!dispatched_at) {
      const d = new Date();
      d.setDate(d.getDate() - 3);
      dispatched_at = d.toISOString().split('T')[0];
    }
  }

  return await sql`
    UPDATE orders 
    SET status = ${status}, dispatched_at = ${dispatched_at}, delivered_at = ${delivered_at} 
    WHERE id = ${id} 
    RETURNING id
  `;
}
