...
import { shouldConfirmPaymentFromWorkflow } from './payment-guard';
...
export async function updateOrderStatus(id, status) {
  const [order] = await sql`
    SELECT order_value, payment_provider, payment_status FROM orders WHERE id = ${id}
  `;
  if (!order) throw new Error('Order not found');
  const confirmsManualPayment = shouldConfirmPaymentFromWorkflow(order, status);
  ...
  return await sql`
    ...
    SET
      status = ${status},
      payment_status = CASE WHEN ${confirmsManualPayment} THEN 'manual' ELSE payment_status END,
      paid_at = CASE WHEN ${confirmsManualPayment} THEN COALESCE(paid_at, CURRENT_TIMESTAMP) ELSE paid_at END
    WHERE id = ${id}
  `;
}
...
export async function updateShipmentStatus(id, status) {
  ...
  const confirmsManualPayment = shouldConfirmPaymentFromWorkflow(shipment, orderStatus);
  await sql`
    UPDATE orders
    SET
      status = ${orderStatus},
