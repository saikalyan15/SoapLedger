...
export async function getAllEssentialOils() {
  return await sql`
    ...
    ORDER BY is_frequently_used DESC, name ASC
  `;
}

export async function setFrequentlyUsed(id, value) {
  const [row] = await sql`
    UPDATE essential_oils
    SET is_frequently_used = ${!!value}
    WHERE id = ${id}
    RETURNING *
  `;
  return row;
...
export async function autoPromoteFrequentOils() {
  return await sql`
    UPDATE essential_oils
    SET is_frequently_used = TRUE
    WHERE is_frequently_used = FALSE
      AND id IN (
        SELECT peo.essential_oil_id
        FROM product_essential_oils peo
        JOIN order_items oi ON oi.product_id = peo.product_id
        GROUP BY peo.essential_oil_id
        HAVING COUNT(DISTINCT oi.order_id) >= ${FREQUENTLY_USED_THRESHOLD}
      )
  `;
}
