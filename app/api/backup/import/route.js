...
export async function POST(request) {
  ...
  try {
    ...
    const queries = [
      ...
      // products (no FK)
      ...products.map(r => sql`
        INSERT INTO products (id, name, base_type, weight_grams, unit_price, is_active, is_seasonal, created_at, ingredients, slug, short_description, image_url, in_stock, is_featured, category, price_range, display_order)
        VALUES (${r.id}, ${r.name}, ${r.base_type}, ${r.weight_grams}, ${r.unit_price}, ${r.is_active}, ${r.is_seasonal}, ${r.created_at}, ${r.ingredients ?? null}, ${r.slug ?? null}, ${r.short_description ?? null}, ${r.image_url ?? null}, ${r.in_stock ?? true}, ${r.is_featured ?? false}, ${r.category ?? null}, ${r.price_range ?? null}, ${r.display_order ?? null})
      `),
      ...
      // orders (FK: customers)
      ...orders.map(r => sql`
        INSERT INTO orders (id, customer_id, order_date, order_value, shipping_charge, packaging_cost, material_cost, status, notes, created_at, dispatched_at, delivered_at)
        VALUES (${r.id}, ${r.customer_id}, ${r.order_date}, ${r.order_value}, ${r.shipping_charge}, ${r.packaging_cost}, ${r.material_cost ?? 0}, ${r.status}, ${r.notes ?? null}, ${r.created_at}, ${r.dispatched_at ?? null}, ${r.delivered_at ?? null})
      `),
