...
export default async function CustomLabelsPage() {
  const [products, businessConfig] = await Promise.all([
    sql`
      SELECT
        p.id, p.name, p.base_type, p.weight_grams, p.ingredients, p.mini_label_description,
      ...
      GROUP BY p.id, p.name, p.base_type, p.weight_grams, p.ingredients, p.mini_label_description
